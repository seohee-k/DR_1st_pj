...
def run_gui(node: TurtleBotYoloGui):
    ...
    def update_view():
        ...
        if node.latest_mask is not None:
            ...
            # === 노란 실선 인식범위(ROI) 시각화 ===
            h, w = node.latest_mask.shape
            roi_y1 = int(h * 0.6)
            roi_y2 = h
            left_x1, left_x2 = 0, int(w * 0.2)
            right_x1, right_x2 = int(w * 0.8), w

            # 왼쪽 ROI 사각형 (파란색)
            cv2.rectangle(mask_bgr, (left_x1, roi_y1), (left_x2, roi_y2), (255, 0, 0), 2)
            # 오른쪽 ROI 사각형 (빨간색)
            cv2.rectangle(mask_bgr, (right_x1, roi_y1), (right_x2, roi_y2), (0, 0, 255), 2)
            # 전체 ROI(하단 40%) 테두리 (초록색, 점선)
            cv2.rectangle(mask_bgr, (0, roi_y1), (w, roi_y2), (0, 255, 0), 1, lineType=cv2.LINE_AA)
...
# ...existing code...
    ...
    def update_view():
        ...
        if node.latest_mask is not None:
            ...
            # === 노란 실선 인식범위(ROI) 시각화 ===
            h, w = node.latest_mask.shape
            # ROI: 하단 40% (y: int(h*0.6) ~ h), 좌우 20% (x: 0~0.2w, 0.8w~w)
            roi_y1 = int(h * 0.6)
            roi_y2 = h
            left_x1, left_x2 = 0, int(w * 0.2)
            right_x1, right_x2 = int(w * 0.8), w
